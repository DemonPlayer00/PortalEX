...
- [x] Binder 外周传感器模拟（**默认开**，可在设置页关闭）：在**系统框架层**伪造外周传感器
      （步频 / 加速度 / 角度 / 指南针）。只 hook 系统框架、**不 hook 目标应用**，
...
- 只能从数据合理性入手，例如：
  ...
  - 开启「Binder 外周传感器模拟」后，被接管类型的**全部**数据都来自模块自产的虚拟世界：
    设备的真实晃动不会出现在加速度计上（这是「完全隔离」的代价，也是最直接的检测面）；
    各传感器之间仍然自洽，只能通过与物理世界的对照（晃动设备、比对真实磁环境）识别；
