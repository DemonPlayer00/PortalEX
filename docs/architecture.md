...
### 3.1 路线推进在 system_server（2026-09-12 起）

- **动因（实测）**：App 是普通应用进程，退后台会被 Cached Apps Freezer 冻结 —— MI6/LineageOS 15 上
  切后台后运动循环整段停摆（25.6s / 25.8s / 47.7s 空洞），期间 system_server 照常按保活节奏推帧，
  但位置一动不动、帧内 `vel≈0` ⇒ 目标跑步应用的配速曲线上每 ~25s 一根尖峰。
- **分工**：App 在起播时把**展开后的整条折线**（平滑段已采样，~1m/点）一次性交给 system_server
  （`set_route` + `route_start`），之后**不再逐 tick 推送坐标**；`hooks/RouteDriver` 按
  `speed × 真实Δt` 推进（单次上限 3s），插值出位置与切线朝向，走**唯一落点**写坐标并投递一帧。
- **几何只有一个来源**：曲线形状由 App 的 `buildPath` 定义，模块只做线性插值（`RouteMath`）——
  两边不可能出现"两条不同的路线"。
- **停机语义**：`route_stop` 必须挂在所有该停的路径上 —— 关悬浮窗（`stopMotionForFloatingHidden`）、
  换/清路线、停会话（模块侧 `stop` 命令也会停）、以及 **App 每次握手时先发一次 `route_stop`**
  （App 被杀时驱动不会跟着死，这是特性；但下次启动已不知道在跑哪条路线，必须显式收掉，
  否则就是"没人操作、位置自己在走"的幽灵推进）。
- **兼容**：旧版模块不认识 `set_route` ⇒ App 明确提示"模块版本较旧（重启后生效）"并回退到
  App 侧逐 tick 推送，不做假成功。
