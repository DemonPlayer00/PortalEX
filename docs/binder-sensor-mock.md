...
* **相位是"步幅相位"：一个波形周期 = 两步**（真实步态的步幅周期），每步只推进半个周期。
* **竖直**：`A·(0.88·sin φ + 0.72·sin(2φ + 1.2))`，`A = 0.35 × v`（封顶 2.2 m/s²）。
  基频（1 周期 = 2 步）是**频谱主峰**，二次谐波（1 周期 = 1 步）保证**每步仍有一个模长峰**；
  两个峰不等高（归一化后 1.005 / 0.696，主峰与旧波形同幅度，阈值行为不变）。
* 这一版是为了修**"基频落在步频上"**带来的读数翻倍：真机走路时躯干竖直振荡的基频就是
  "两步一次"，按"两步一周期"换算步频的估计器（FFT 取基频 ×2 那类）读到 2× 步频
  ——实测反馈"步频 286"（真值约 143）正是这个形态。把波长拉长一倍后，这类估计器读数回落。
* 只拉长波长、不补二次谐波是不行的：一个周期里就只剩一个极值，**峰值/阈值类**估计器会
  **少计一半**（把一个错误换成另一个）。两组系数都做过数值核对（扫过一个步幅周期取极值 +
  频谱主峰）：`0.72/0.28` → 每周期 1 个极值（不合格）；`0.88/0.72` → 2 个不等高极值且主峰仍是基频。
* **前后**：`GAIT_AY_RATIO(0.25)·A·同相波形(±0.05rad)`
* **左右**：`GAIT_AX_RATIO(0.12)·A·同相波形(±0.05rad)`（三分量几乎同相 ⇒ 模长形状与 az 一致，
  否则模长会出现额外过阈值，阈值类检测器多计一步——实测踩到过）
* 静止时三项归零；`GRAVITY` 保持恒定 `(0,0,9.81)`，`ACCELEROMETER = GRAVITY + LINEAR_ACCELERATION`
  严格成立（真机的物理关系）。**步事件率不变**（仍是一步一个 counter/detector 事件，
  由 Java 侧按步频公式积分），改的只是 IMU 波形的口径。

实测（Android 16 + 高通/OPPO，按住悬浮摇杆模拟行走，`sensorprobe` 原始 az 采样，166Hz）：

```
11.39 11.48 11.40 11.22 10.97 10.68 10.41 10.20 10.06 10.05 10.12 10.27 10.48 10.69 10.86
10.94 10.90 10.71 10.39  9.93  9.37  8.76  8.28  7.79  7.41  7.04  7.03  7.21  7.75  8.26
 8.85  9.49 10.10 10.74 11.14 11.38 11.47 11.41 11.23 10.96
```

一个步幅周期（约 35 个采样点）里**两个不等高的峰**：主峰 11.48 / 次峰 10.94（≈ 0.70），
峰谷 ~7.0 —— 主峰幅度与旧波形一致，步幅频率即基频。`status()` 里的 `gait=` 字段
（Test 页可见）会打印当前口径，便于确认跑的是哪一版。

历史上的三次踩坑（都保留在代码注释里）：对称 `sin(2φ)`（每步两个等高峰，基频被读成 2× 步频）；
