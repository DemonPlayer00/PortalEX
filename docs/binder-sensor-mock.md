...
| 量 | 修前 \|Δ\|中位 | 修后 \|Δ\|中位 | 判读 |
| --- | --- | --- | --- |
| 加速度计 | 0.89 | **0.047 m/s²** | 平滑 ✓ |
| 重力 | 0.87 | **0.045 m/s²** | 平滑 ✓ |
| 线性加速度 | 0.004 | 0.007 m/s² | 平滑 ✓ |
| 朝向 | 0.14° | 0.11° | 平滑 ✓ |
| 罗盘角 | 7.9° | **1.28°**（最大 16°） | 可接受 |
| 磁场（幅度） | 4.4 | 1.26 µT | 仍偏毛糙（方向不受影响） |
| **恒等式残差** | 1.70 | **0.004** | `accel = gravity + linear` 成立 ✓ |
...
⚠️ **改这里的教训**：`virtual_world.c` 里取偏差那一行曾被**两次"没命中的替换"**漏掉
（旧口径 `vw_wobble_dev` 一直生效，而 .so 照常编译成功）。证据是**真机数据两轮完全不变**。
改这一行务必 grep 核验符号，别信"构建通过"。
