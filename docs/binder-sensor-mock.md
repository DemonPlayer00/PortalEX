...
| 量类 | 谁的 | 逐条随机怎么施加 |
| --- | --- | --- |
| **角度类**：朝向 / 旋转矢量 / 磁场**方向** | 由同一个方位角派生 | 慢漂按 180° 折算；逐条随机**最多 1°**，且 **100ms 采样保持**（同窗内朝向/磁场/旋转矢量拿到同一个角度偏差）⇒ 三者一致、指针不跳 |
| **磁场幅度** | 只抖**幅度** | 按 `1+dev` 缩放整条磁场矢量 —— 逐分量加偏差会让**方向**被噪声主导（实测罗盘角 |Δ| 中位 61°，而真实只有 ±0.2°） |
| **加速度计 / 重力** | 同一个"重力模型" | 加同一个**保持窗内**的偏差（100ms 采样保持）⇒ `加速度计 = 重力 + 线性加速度` 仍然成立 |
| **线性加速度** | 走路残差 | **不吃偏差**（吃了恒等式就破） |

真机实测（Test 页「IMU 平滑度」，静止 25s / 走动 35s 各一轮）：

| 量 | 静止 \|Δ\|中位 | 走动 \|Δ\|中位 | 判读 |
| --- | --- | --- | --- |
| 朝向 | 0.14° | 0.18° | 平滑 ✓ |
| 线性加速度 | 0.004 | 0.006 m/s² | 平滑 ✓ |
| 加速度计 | 0.89 | 1.11 m/s² | 毛糙 ✗（±15%×1g 的台阶） |
| 重力 | 0.87 | 0.78 m/s² | 毛糙 ✗（同上） |
| 磁场 | 4.4 | 5.0 µT | 毛糙 ✗（幅度抖动 15%×50µT） |
| 罗盘角 | 7.9° | 9.9° | 慢漂主导（±27° 的 1.5s 游走），逐条抖动已 ≤1° |

⚠️ **判读口径**：Test 页那个"恒等式残差"的**对齐逻辑还有 bug**（容差 60ms 内只配上 15%，
而实测最近时间差中位 13.6ms）—— 那一行的数字**不可信**，别拿它下结论。
按构造它现在应当成立（同一个保持窗偏差加在加速度与重力上，线性不加）。
