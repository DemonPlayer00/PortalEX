...
/**
 * **虚拟世界**：被模拟的那份「世界状态」+ 它的生成器（坐标 / 速度 / 朝向 / 游走相位）。
 *
 * 与 [LocConfig] 的分工：
 *  · [LocConfig] = 用户设置，只在握手 / 改设置 / 命令下发时写；
 *  · 本对象      = 运行时状态，每帧都在变（位置帧、传感器事件都在读它）。
 * 原先两者混在同一个 `object FakeLoc` 里，看不出哪些字段是「配置」、哪些是「本帧状态」。
 *
 * 约定：
 *  · 对外公开名字仍是 `FakeLoc.xxx`（[FakeLoc] 是门面，直接转发到这里）；
 *  · 这里的代码是**状态与推算**，纯逻辑：不发回调、不装 hook、不读系统设置；
 *  · 需要配置时直接读 [LocConfig]（不经过门面，避免"自己调自己"）；
 *  · 语义文档随代码一起搬到这里（原注释逐字保留），[FakeLoc] 侧只留门面摘要。
 *
 * 与原生层的对应：`xposed/src/main/cpp/virtual_world.c` 是同一模型在 sensor HAL 侧的
 * C 实现（朝向快频段 / 步频 / 摆动同构），两边改动要对齐。
 */
internal object VirtualWorld {
    ...
    fun speedFloorSample(): Double {
        ...
        if (dt > 0.0) {
            if (Random.nextDouble() < 1.0 - exp(-dt / SPEED_FLOOR_RETARGET_TAU)) {
                speedFloorTarget = LocConfig.speedFloor * (0.6 + Random.nextDouble() * 0.8)
            }
            speedFloorSmoothed += (speedFloorTarget - speedFloorSmoothed) *
                (1.0 - exp(-dt / SPEED_FLOOR_SMOOTH_TAU))
        } else {
            speedFloorTarget = LocConfig.speedFloor
            speedFloorSmoothed = LocConfig.speedFloor
        }
    ...
    fun cadenceForSpeed(speed: Double): Int {
        ...
        if (LocConfig.cadenceScale == 1.0) return base
    ...
    fun moveLocation(lat: Double, lon: Double, n: Double, angle: Double): Pair<Double, Double> {
