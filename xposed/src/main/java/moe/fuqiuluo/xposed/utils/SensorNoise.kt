 ...
 *
 * 口径：
 *  - 噪声在校准时按轴采集**中位数 μ 与方差 σ²**；使用时按 **高斯分布 N(0, σ²) 逐事件生成动态值**。
...
object SensorNoise {
    ...
    /**
     * 默认 σ 取真机静止的量级（≈ 陀螺 1e-3 / 加速度 6e-3 / 磁场 0.1~0.3），陀螺零偏默认 0；
     * 与 Calibration 页「恢复默认」用的是同一份值。
     */
    ...
    val ITEMS: List<Item> = listOf(
        Item(
            "陀螺仪噪声 σ", "rad/s",
            "逐轴标准差（x/y/z）。真机静止量级 σ≈0.001",
            GYRO, Shape.XYZ, 0.05f, 4,
        ),
        Item(
            "陀螺仪零偏 μ", "rad/s",
            "逐轴零偏，可为负。陀螺的零参考物理上就是 0，实测中位数即真实零偏；一键校准会填这一行",
            GYRO_BIAS, Shape.XYZ, 0.5f, 4,
        ),
        Item(
            "加速度计噪声 σ", "m/s²",
            "逐轴标准差。真机静止量级 σ≈0.007",
            ACCEL, Shape.XYZ, 0.5f, 4,
        ),
        Item(
            "重力噪声 σ", "m/s²",
            "逐轴标准差。重力是低通后的结果，真机比加速度计安静得多（σ≈0.001）",
        ...
        Item(
            "磁场噪声 σ", "µT",
            "逐轴标准差。真机静止量级 σ≈0.2；受环境影响大，建议在常用环境下校准",
            MAG, Shape.XYZ, 5f, 3,
        ),
        Item(
            "方向角噪声 σ", "°",
