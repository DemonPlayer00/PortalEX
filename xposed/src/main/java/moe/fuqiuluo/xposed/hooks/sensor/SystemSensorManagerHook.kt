/*
 * ═══════════════════════════════════════════════════════════════════════════════
 *  【已冻结】应用侧传感器 hook —— “真实回调改写方案”
 *  冻结日期：2026-09-11      依据：用户裁决「暂时注释相关 hook 代码」
 * ═══════════════════════════════════════════════════════════════════════════════
 *
 * 它曾经做什么：注入每个勾选的应用进程，把 SystemSensorManager 的真实回调数据
 * 改写成模拟值（步数按速度推进、航向族共用同一虚拟方位）。**不自产事件**，
 * 完全依附真实回调——所以设备不走路时它没有任何数据源，必须自己积分/合成。
 *
 * 为什么冻结（全部有实测，见 ~/.dsh/memory/PortalEX/2026-09-11.md 与 docs）：
 ...
 *     意图 190 步/分 ↔ 原生 step_rate 192/min ↔ 框架缓存事件间隔 ↔ 客户端逐条一致，
 *     且**不需要目标应用进作用域**。两套同时跑只会互相打架，所以先冻结这一套。
 *
 * 恢复方式（三步，缺一不可）：
 *  1. 先补**开关传播通道**，否则会重现上面第 1、2 条：推荐 = 框架侧发布健康度/模式
 *     （fw:ok | fw:down:<原因>）+ 运行时让位（到货新鲜就不改写、直接透传，静默才接管）。
 *  2. 解冻本文件：删掉这层外层块注释与上面的存根，恢复原实现。
 *  3. FakeLocation.kt 的应用进程分支里，把 SystemSensorManagerHook(lpparam.classLoader)
 *     那行按开关条件恢复安装。
 *
 * 文件现状：上方存根只为让 Test 页那句“应用侧注入痕迹”有个明确答复；
 * 原实现全文以块注释形式原样保留在文件末尾（Kotlin 块注释可嵌套，勿删）。
 * ═══════════════════════════════════════════════════════════════════════════════
 */
 ...
 *
 * Test 页（TestFragment）会调用 [stepTraceText] 展示“应用侧注入痕迹”。冻结后没有痕迹可言，
 * 这里直接给出结论，避免 UI 上出现空白或误以为探针坏了。
 */
object SystemSensorManagerHook {

    /** 冻结说明（Test 页原样展示，保持单行简短） */
    fun stepTraceText(): String =
        "（应用侧传感器 hook 已冻结：模拟只由系统框架侧完成，见 SystemSensorManagerHook.kt 顶部 FREEZE 说明）"
}

/*
 * ═══════════════════════════════════════════════════════════════════════════════
 *  以下是**冻结的原实现全文**（原样保留，未做任何逻辑修改）。
 *
 *  解冻方式：删掉本外层注释块的起始行与文件末尾的结束行，并删掉上方存根即可。
 *  注意：原实现里的文件级注解与 package 声明也在注释内，解冻后它们会回到文件头。
 * ═══════════════════════════════════════════════════════════════════════════════
