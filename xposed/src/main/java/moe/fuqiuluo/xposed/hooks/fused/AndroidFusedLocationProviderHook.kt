...
object AndroidFusedLocationProviderHook: BaseLocationHook() {
    operator fun invoke(classLoader: ClassLoader) {
        val where = if (FakeLoc.isSystemServerProcess) "system_server" else "fused-process"
...
//        }
        ...
        val blindCount = BlindHookLocation(cChildLocationListener, classLoader)
        FusedStatus.markChildListener(where, blindCount)
        FusedStatus.logIfDebug()
    }
}