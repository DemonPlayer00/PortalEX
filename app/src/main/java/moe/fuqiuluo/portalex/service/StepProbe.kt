 ...
 */
/**
 * **仅诊断用**：本文件不是产品功能的一部分，只服务于 Test 页的"普通应用视角"一栏
 * （真订阅 + 轮询总步数，模拟普通计步应用会看到什么）。
 *
 * 维护约定：它读的是**应用侧**传感器回调（app 侧 hook 已冻结 ⇒ 只有系统框架侧在注入），
 * 因此这里的变化只反映"系统侧喂给了客户端什么"，不参与任何生成/投递决策。
 * 若某天要精简产品路径，这个文件是第一候选（连同 Test 页对应栏目一起删）。
 */
object StepProbe {
